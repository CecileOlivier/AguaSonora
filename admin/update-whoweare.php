...
<html lang="fr">
    <head>
        <meta charset="UTF-8">
        <title>Agua Sonora : Modification du texte de présentation</title>
    ...
    <body>
        <header>
            <h1>Espace d'administration - Modification du texte de présentation</h1>
...
function form_valid($texte) {
    ...
    if(mb_strlen($texte, 'utf8') > 2000) {
        $erreurs[] = 'le texte ne peut pas faire plus de 2000 caractères';
    }
...
if(isset($_GET['id'])) {
    ...
    if($presentation) {
        ...
        if(isset($_POST['texte'])) {
            ...
            if($form_valid === true) {
                    // on insère
                    $updatetexte = update_texte($id, $texte);
                    if($updatetexte === true) {
