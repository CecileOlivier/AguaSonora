...
<html lang="fr">
    <head>
        <meta charset="UTF-8">
        <title>Agua Sonora : Modification de la galerie d'images</title>
    ...
    <body>
        <header>
            <h1>Espace d'administration - Modification de la galerie d'images</h1>
    ...
    if($pictures) {
        ...
        if(isset($_POST['Modifier'])) {
            ...
            if($form_valid === true) {
                ...
                if($upload_ok) {
                    // on insère
                    $pictures_update = update_picture($id, $nom);
                    if($pictures_update === true) {
                        // succès ! \o/
                        ...
                        echo '<p class="error">Il y a eu une erreur dans la mise à jour : '.$pictures_update.', veuillez réessayer.</p>'.PHP_EOL;
                    }
                } else {
        ...
        if($affichage_formulaire) {
            ...
            <form id="update" method="post" action="" enctype="multipart/form-data">
                ...
                <p><img src="../img/slider/<?= $nom ?>" alt="" width="600px"/></p>
