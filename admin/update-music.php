...
<html lang="fr">
    <head>
        <meta charset="UTF-8">
        <title>Agua Sonora : Mise à jour des morceaux</title>
    ...
    <body>
        <header>
            <h1>Espace d'administration - Mise à jour des morceaux</h1>
        ...
        if(isset($_POST['Modifier'])) {
            ...
            if($form_valid === true) {
                ...
                if($fichier !== null && is_uploaded_file($fichier['tmp_name'])) {
                    ...
                    if(move_uploaded_file($fichier['tmp_name'], '../audio/'.$nom)) {
                    } else {
                        $upload_ok = false;
                    }
                }
                if($upload_ok) {
                    // on insère
                    $music_update = update_music($id, $nom);
